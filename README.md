...
| [`canopy/no-tolocalestring-for-dates`](docs/rules/no-tolocalestring-for-dates.md) | Disallows `.toLocaleDateString()`, `.toLocaleTimeString()`, and `Intl.DateTimeFormat` for display. Use Luxon `DateTime` with a Canopy preset such as `DateTime.DATE_SHORT`. `.toLocaleString()` is **not** reported — it is Luxon's own correct API and is also how numbers get thousands separators. |
