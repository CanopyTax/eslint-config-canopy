...
| [`canopy/comment-length`](docs/rules/comment-length.md) | Limits a comment to a configurable length (default 240 characters, ~2-3 lines). A run of consecutive `//` lines is measured together, so switching prose to stacked `//` lines does not evade the limit; each `/* */` block is measured whole. Exempt are structured JSDoc blocks (a `/** */` carrying at least one `@tag`) and any comment containing a URL; a tagless doc block and a directive comment are not, so a genuinely long comment otherwise needs an explicit `eslint-disable`. |
| [`canopy/no-external-ref-in-comment`](docs/rules/no-external-ref-in-comment.md) | Disallows references in a comment that only resolve outside the repo — a ticket ID, a section of a planning document, a PR number. The rule checks each comment group, including tagged JSDoc and the reason text of a directive comment, since the explanation should be understandable to a reader who has only the repo open. |
| [`canopy/no-history-in-comment`](docs/rules/no-history-in-comment.md) | Disallows a comment that describes how the code used to behave or what a fix changed. A reader of the current code cannot see the earlier state, so the comment should describe how the code works now. The rule checks each comment group, including directive reason text, for a case-insensitive, whole-phrase match against a fixed phrase list. |
| [`canopy/no-hedge-in-comment`](docs/rules/no-hedge-in-comment.md) | Disallows hedging language in a comment — "not sure why," "hopefully," "I think" — that signals the author never confirmed the claim. The rule checks each comment group, including directive reason text, for a case-insensitive, whole-phrase match against a fixed phrase list. |
| [`canopy/no-banner-comment`](docs/rules/no-banner-comment.md) | Disallows a decorative banner made of five or more repeated characters (`= * # ~ - ─ ━`), such as a divider line or a boxed title, since it takes up lines without adding information and its length stops matching the title once either is edited. File structure, section naming, and `describe` blocks separate sections without that maintenance. |
| [`canopy/no-obvious-comment`](docs/rules/no-obvious-comment.md) | Warns on a comment that just restates the code below it in different words, adding reading time without adding information. This is a heuristic based on word overlap between the comment and the code's identifiers, so it ships at **warn** rather than **error** and its findings are advisory. |
