...
| [`canopy/no-hardcoded-font-size`](docs/rules/no-hardcoded-font-size.md) | Disallows font sizes outside the `cp-*` typography scale: Tailwind `text-xs`…`text-9xl`, arbitrary lengths like `text-[13px]`, and literal `fontSize` values in `style` props. Colours, alignment, and decoration sharing the `text-` prefix are not reported, nor are computed values or `inherit`. |
