...
| [`canopy/no-history-in-comment`](docs/rules/no-history-in-comment.md) | Disallows a comment that narrates how the code used to behave or what a fix changed, since that narration rots the moment the reader can't see the "before" state. The rule checks each comment group, including directive reason text, for a case-insensitive, whole-phrase match against a fixed phrase list. |
