...
| [`canopy/no-banner-comment`](docs/rules/no-banner-comment.md) | Disallows a decorative banner made of five or more repeated characters (`= * # ~ - ─ ━`), such as a divider line or a boxed title, since it adds visual weight without information and drifts out of sync with the title it surrounds. File structure, section naming, and `describe` blocks carry the same organizing intent without the upkeep. |
