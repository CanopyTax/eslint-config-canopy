...
| [`canopy/no-raw-fetch`](docs/rules/no-raw-fetch.md) | Disallows the ambient `fetch()` global and `axios` imports in favour of `fetcher!sofe`, which supplies auth headers, CSRF, error routing and Sentry breadcrumbs. A locally defined or imported `fetch` is not reported. **Enable per-repo:** disable it in `frontend-node-server`, `fetcher`, and other server-side packages, where `fetch()` is correct. |
