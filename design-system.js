...
// Understory's theme.css redefines gray onto its own variables, but the name
// collides with Tailwind's default gray, so no-raw-colors would report it.
...
const understoryGray = colorUtilities.map((u) => `${u}-gray-*`);
...
export default [
  {
    ...
    rules: {
      ...
      'shadcn/no-raw-colors': ['error', { allow: understoryGray }],
